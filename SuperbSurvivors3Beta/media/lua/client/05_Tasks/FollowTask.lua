...
function FollowTask:needToFollow()
	...
	if distance > GFollowDistance + self.FollowDistanceOffset 
		...
		or (self.parent:Get():getVehicle() -- Why just being in a vehicle was a standlone condition for needing to follow makes no sense to me
		and self.FollowChar:getVehicle() ~= self.parent:Get():getVehicle())
	then
		-- CreateLogLine('NPC Follow', true, tostring(self.parent:getName()) .. " has Task need follow determination is true")
