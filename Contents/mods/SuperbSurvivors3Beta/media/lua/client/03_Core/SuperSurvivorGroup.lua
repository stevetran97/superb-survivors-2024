...
function SuperSurvivorGroup:UseWeaponType(thisType)
	...
	for i = 1, #members do
		...
		elseif members[i].reEquipMelee ~= nil then
			members[i]:reEquipMelee();
		end
	end
end
...
function SuperSurvivorGroup:getClosestIdleMember(ofThisRole, referencePoint)
	...
	for i, MemberId in pairs(self.Members) do 
		...
		if MemberId ~= 0 and workingID then
...
function SuperSurvivorGroup:getMembersInRange(referencePoint, range, isListening)
	...
	for i, MemberId in pairs(self.Members) do 
		...
		if MemberId ~= 0 and workingID and SSM:Get(workingID) then
			...
			if distance <= range
				and (not isListening or 
					SSM:Get(workingID):getCurrentTask() == "Listen")
			then
				table.insert(TableOut, SSM:Get(workingID));
			end
		end
...
function SuperSurvivorGroup:PVPAlert(attacker)
	...
	for i, MemberId in pairs(self.Members) do 
		...
		if MemberId ~= 0 and ss then
			local member = SSM:Get(workingID):Get();
			if workingID and member and member:CanSee(attacker) then
